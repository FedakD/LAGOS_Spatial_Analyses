...
```{r}
site_count_200 <- chla_secchi %>%
  group_by(lagoslakeid) %>%
  mutate(count = n()) %>%
  distinct(lagoslakeid, .keep_all=T)
  
  
spatial_site_count_200 <- inner_join(spatial_lakes,site_count_200 %>%
                                   distinct(lagoslakeid,.keep_all=T),
                                 by='lagoslakeid')

state_sites_200 <- st_join(spatial_site_count_200, us_states())

state_site_count_200 <- state_sites_200 %>%
  group_by(name) %>%
  mutate(observations = sum(count)) %>%
  distinct(name, .keep_all=T)
...
ggplot(state_site_count_200, aes(x=state_abbr, y=observations)) +
  geom_point() +
  xlab("State") +
  ylab("# of Secchi Disk Observations")

mapview(state_site_count_200)
